...
namespace Solution
{
    ...
    class CommandListContents : Command
    {
        ...
        public override Error validateParameters()
        {
            ...
            {
                if (commandNoParameters.Length == 3)
                {
                    //Extra
                    if (commandNoParameters[2].ToString().Equals("1"))
                    {
                        //Valid for extra
                    }
                    else
        ...
        public override bool Execute()
        {
            ...
            if (commandNoParameters.Length > 2)
            {
                //Extra
                extra = true;
                var dirs = commandNoParameters[1].Split('/');
                string actual = Directory.GetCurrentDirectory();
                foreach (var d in dirs)
                {
                    string dirGoto = actual + "\\" + d;
                    if (!Directory.Exists(dirGoto))
                    {
            ...
            if (commandNoParameters.Length > 1 && extra==false)
            {
